...
The class :class:`CloudToMorphologyIntersection <.connectivity.point_cloud.morphology_cloud_intersection.MorphologyToCloudIntersection>` creates connections between the points of the morphology of the presynaptic cell and a point cloud representing a postsynaptic cell, checking if the points of the morphology are inside the geometric shapes representing the postsynaptic cells.
...
A `ShapesComposition` can be read from a file using the method :meth:`~bsb.connectivity.point_cloud.geometric_shapes.ShapesComposition.load_from_file`.
