    ...
    def _set_cells(self, label=[[], []]):
        ...
        for t in types:
            ...
            for cell_type in self.__dict__[t + "_types"]:
                ...
                ids.sort()
                if label[0] and t == "from_cell":
                    labelled = self.scaffold.get_labelled_ids(label[0]).tolist()
                    labelled.sort()
                ...
                elif label[1] and t == "to_cell":
                    labelled = self.scaffold.get_labelled_ids(label[1]).tolist()
                    labelled.sort()
                    # Compute intersect of sorted list
                    label_slice = compute_intersection_slice(ids, labelled)
                    # Store the labelled cells of the type.
